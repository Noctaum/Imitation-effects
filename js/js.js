...
//Постройка таблицы
 function builder (){
	...
	if(rowsForT === ""){
 	...
 	var ij = 0;
 	var str = "n";
 	$('#firstFirst').append('<h4>Параметры F и t достаточно ввести 1 раз в одну из таблиц.</h4>');
		...
		for(var i=0; i<(+rowsForI); i++){
		    ...
		    for(var j=0; j<3; j++){
		    	if(i === 0) {
		    		switch (j){
		    			case 0:
			    			var colHead = $('<td><h5>№ значения</h5></td>');
...
//Строим график P=f(I)
	...
	$('#third').append('<div class="nameOfShedule"><h3>График зависимости P=f(F)</h3></div>');
...
//Нумеруем ось X
	...
	graph.moveTo(valueToX(0,maxX), valueToY(maxY,maxY));
	graph.lineTo(valueToX(0,maxX), valueToY(0,maxY));
	graph.stroke();
...
//Нумеруем ось Y
	...
	graph.beginPath();
	graph.strokeStyle = "black";
	graph.moveTo(valueToX(0,maxX), valueToY(0,maxY));
	graph.lineTo(valueToX(maxX,maxX), valueToY(0,maxY));
	graph.stroke();
	    ...
	    for(var j=0; j<3; j++){
	    	...
	    	switch (j){
	    		case 0:
	    			...
	    			break;
	    		case 1:
	    			var col = $('<td>'+((+massNewP[i]).toFixed(3))+'</td>');
	    			row.append(col);
	    			break;
	    		case 2:
	    		var col = $('<td>'+((+massNewPPP[i]).toFixed(3))+'</td>');
	    			row.append(col);
	    			break;
	    	}
	    }
